...
MainWindow::MainWindow() {
    ...
    avconfig_.bitrate_bps = 4000000;
    avconfig_.framerate = 25;
    avconfig_.codec = "";
}
...
bool MainWindow::Create() {
    ...
    window_ = SDL_CreateWindow("Screen Live", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, window_width_,
                               window_height_, window_flag);
...
void MainWindow::Process(SDL_Event &event) {
    if (IsWindow()) {
        if (overlay_) {
...
bool MainWindow::UpdateARGB(const uint8_t *data, uint32_t width, uint32_t height) {
    ...
    if (texture_format_ != SDL_PIXELFORMAT_ARGB8888 || (texture_width_ != width) || (texture_height_ != height)) {
        if (texture_) {
            SDL_DestroyTexture(texture_);
            texture_ = nullptr;
        }
...
bool MainWindow::StartLive(int &event_type, std::vector<std::string> &encoder_settings,
    ...
    if (avconfig_ != avconfig) {
        ...
        overlay_->SetLiveState(SCREEN_LIVE_RTSP_SERVER, false);
        overlay_->SetLiveState(SCREEN_LIVE_RTSP_PUSHER, false);
        overlay_->SetLiveState(SCREEN_LIVE_RTMP_PUSHER, false);
