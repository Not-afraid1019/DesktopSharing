...
#ifndef DESKTOPSHARING_SCREENLIVE_H
#define DESKTOPSHARING_SCREENLIVE_H
...
struct AVConfig {
    ...
    bool operator !=(const AVConfig& src) const {
        if (src.bitrate_bps != bitrate_bps || src.framerate != framerate || src.codec != codec) {
            return true;
        }
        return false;
    }
...
#endif //DESKTOPSHARING_SCREENLIVE_H
